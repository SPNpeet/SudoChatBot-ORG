...
/**
 * ตัวจัดการจริงของการเข้าสู่ระบบด้วย LINE
 *
 * ⚠️ แยกออกมาเป็นฟังก์ชันเพราะต้องถูกเรียกจากสองเส้นทาง (28 ส.ค. 2569)
 * LINE ยอมรับเฉพาะ callback URL ที่ลงทะเบียนไว้ในคอนโซลเท่านั้น
 * ของโปรเจกต์นี้ลงทะเบียนไว้แค่ /api/line/callback (ใช้กับการผูก LINE OA)
 * ส่วน /api/auth/line/callback ยิงแล้ว LINE ตอบ 400 เพราะไม่ได้ลงทะเบียน
 * (พิสูจน์แล้วด้วยการยิงทั้งสองเส้น: 302 กับ 400)
 *
 * แทนที่จะรอเจ้าของไปเพิ่ม URL ในคอนโซล เราใช้เส้นที่ลงทะเบียนไว้แล้ว
 * แล้วแยกสองงานออกจากกันด้วย state: การผูก OA ใช้ state แบบ shopId.nonce.signature
 * ส่วนการล็อกอินใช้ UUID ที่ต้องตรงกับคุกกี้ line_oauth_state เป๊ะ
 *
 * ⚠️ redirect_uri ตอนแลก token ต้องตรงกับตอนขอ authorize เป๊ะ ๆ ตามข้อกำหนด OAuth
 * จึงต้องส่งเข้ามา ห้าม hardcode
 */
export async function handleLineLoginCallback(request: Request, redirectUri: string) {
