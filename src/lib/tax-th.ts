...
/**
 * ป้ายอัตรา VAT ของ "เอกสารใบนั้น" — คิดย้อนจากตัวเลขที่พิมพ์อยู่บนใบจริง
 *
 * ⚠️ ทำไมไม่ใช้ VAT_PERCENT_LABEL ตรง ๆ บนเอกสาร:
 * ยอด vat_amount ถูกคำนวณด้วยอัตรา ณ วันที่ออกเอกสาร (เก็บใน vat_rates)
 * แต่ป้ายเป็นค่าคงที่ตอน deploy — วันที่อัตราเปลี่ยน ใบกำกับภาษีจะพิมพ์
 * "ภาษีมูลค่าเพิ่ม 7%" คู่กับยอดที่คิดจากอัตราใหม่ = เอกสารขัดแย้งในตัวเอง
 * ซึ่งผู้สอบบัญชี/สรรพากรจับได้ทันทีตาม ม.86/4
 *
 * คิดจากตัวเลขบนใบเอง จึงตรงกับสิ่งที่ผู้อ่านเห็นเสมอ ไม่ว่าอัตราจะเปลี่ยนกี่ครั้ง
 * และไม่ต้องยิงฐานข้อมูลเพิ่มตอนพิมพ์เอกสาร
 */
export function vatPercentLabelOf(doc: { total?: number | string | null; vat_amount?: number | string | null }): string {
  const vat = Number(doc.vat_amount ?? 0);
  const base = Number(doc.total ?? 0) - vat;
  if (!(vat > 0) || !(base > 0)) return VAT_PERCENT_LABEL;   // ไม่มี VAT/ข้อมูลไม่พอ = ใช้ค่ามาตรฐาน
  const pct = Math.round((vat / base) * 1000) / 10;          // ทศนิยม 1 ตำแหน่งพอสำหรับอัตราภาษี
  return `${pct}%`;
}
...
/**
 * อัตราหัก ณ ที่จ่ายที่ใช้กันจริงของแต่ละประเภทเงินได้
 *
 * ใช้ "เตือน" เท่านั้น ไม่ใช่บล็อก เพราะมีกรณีเฉพาะที่อัตราต่างจากนี้ได้จริง
 * (จ่ายต่างประเทศ · มีประกาศเฉพาะเรื่อง · ผู้รับเป็นมูลนิธิ/สมาคม)
 * หน้าที่ของมันคือสะกิดตอนที่คู่ที่กรอกดูไม่เข้ากัน ไม่ใช่ตัดสินแทนผู้ทำบัญชี
 *
 * เจอของจริงในไฟล์ที่ผู้ใช้ส่งมาตรวจ: ประเภทเงินได้ "ค่าบริการ/ค่าจ้างทำของ"
 * แต่กรอกอัตรา 5% ซึ่งเป็นอัตราค่าเช่า — ถ้าไม่มีใครทัก ก็ยื่นผิดไปทั้งเดือน
 */
const WHT_USUAL_RATES: Record<string, { rates: number[]; why: string }> = {
  "40(2)": { rates: [3, 5, 10, 15], why: "ค่านายหน้า/รับทำงานให้ จ่ายนิติบุคคลใช้ 3% · บุคคลธรรมดาคิดตามอัตราก้าวหน้า" },
  "40(3)": { rates: [3, 5, 10, 15], why: "ค่าลิขสิทธิ์ จ่ายนิติบุคคลในประเทศใช้ 3%" },
  "40(4)": { rates: [1, 10, 15], why: "ดอกเบี้ยจ่ายนิติบุคคล 1% · บุคคลธรรมดา 15% · เงินปันผล 10%" },
  "40(5)": { rates: [5], why: "ค่าเช่าทรัพย์สินใช้ 5%" },
  "40(6)": { rates: [3, 5], why: "ค่าวิชาชีพอิสระใช้ 3%" },
  "40(7)": { rates: [3, 5], why: "ค่าจ้างรับเหมาใช้ 3%" },
  "40(8)": { rates: [1, 2, 3], why: "ค่าบริการ/จ้างทำของใช้ 3% · ค่าขนส่ง 1% · ค่าโฆษณา 2%" },
};

/**
 * อัตราที่กรอกเข้าคู่กับประเภทเงินได้ไหม
 * คืนข้อความเตือนถ้าดูไม่เข้าคู่ · คืน null ถ้าปกติหรือประเมินไม่ได้
 */
export function whtRateMismatch(incomeCode: string | null | undefined, rate: number): string | null {
  if (!incomeCode || !(rate > 0)) return null;
  const u = WHT_USUAL_RATES[incomeCode];
  if (!u || u.rates.includes(rate)) return null;
  const label = WHT_INCOME_TYPES.find((t) => t.code === incomeCode)?.desc ?? incomeCode;
  return `อัตรา ${rate}% ไม่ใช่อัตราที่ใช้กันกับ "${label}" — ${u.why}`;
}
