...
export function calcDocTotals(
  ...
  if (vatMode === "exclusive") {
    // ⚠️ ต้องใช้ vatRate (อัตรา ณ วันที่ออกเอกสาร) ไม่ใช่ VAT_RATE (ค่าคงที่ตอน deploy)
    // เดิมใช้ค่าคงที่ ทำให้กลไก "อัตราตามวันที่" ตายทั้งเส้นสำหรับเอกสารแบบบวก VAT เพิ่ม
    // ซึ่งเป็นโหมดที่ใช้มากที่สุด และทำให้ใบลดหนี้ที่ตั้งใจอ้างอัตราของใบเดิมไม่ทำงานจริง
    // (สาขา inclusive ด้านล่างใช้ค่าจากพารามิเตอร์ถูกต้องอยู่แล้ว — เสียเฉพาะสาขานี้)
    //
    // ⚠️ ห้ามเปลี่ยนเป็น base * pct / 100 เพื่อ "ให้เหมือนสาขา inclusive"
    // ทดสอบ 3 ล้านยอดแล้ว: ต่างกัน 1,431 เคส เช่นฐาน 14.50 -> เดิม 1.02 แต่แบบจำนวนเต็มได้ 1.01
    // เพราะ 1.015 ในเลขฐานสองต่ำกว่าค่าจริงนิดเดียว การคูณทศนิยมตรง ๆ จึงปัดครึ่งขึ้นได้ถูกกว่า
    // ที่อัตรา 7% ค่านี้เท่ากับของเดิมเป๊ะ = เอกสารเดิมทุกใบไม่ขยับสักบาท
    vat = r(base * vatRate);
    exVat = base;
    total = r(exVat + vat);
