...
// ============================================================
//  เขตเวลาไทยกับขอบเดือนภาษี (เพิ่ม 6 ส.ค. 2569 — ไม่เคยตรวจ)
//
//  ⚠️ บั๊กชนิดนี้ไม่มีใครเห็นจนกว่าจะยื่นภาษีผิดงวด
//  server รันเป็น UTC แต่ความรับผิดทางภาษีนับตามเวลาไทย (UTC+7)
//  เอกสารที่ออก 23:30 ของวันสิ้นเดือน ถ้าเผลอคิดเป็น UTC จะกลายเป็นเดือนถัดไปทันที
//  = ยอดขายไปโผล่ผิดงวด ภ.พ.30 ทั้งเดือนนั้นและเดือนถัดไป
//
//  และเครื่องหมาย "รู้แค่วันที่" (เก็บเป็นเที่ยงตรงเวลาไทย) ต้องไม่แสดงเวลาปลอม
//  เจ้าของเจอเองเมื่อ 1 ส.ค. 2569: ทุกแถวหน้าการเงินขึ้น "12:00" เรียงกันทั้งหน้า
//  อ่านแล้วเข้าใจผิดว่าโอนเที่ยงตรงพร้อมกันหมด — เวลาปลอมแย่กว่าไม่มีเวลา
// ============================================================
section("เขตเวลาไทยกับขอบเดือนภาษี");
{
  for (const [d, day, mon] of [
    ["2026-01-31", "31", "ม.ค."],
    ["2026-02-28", "28", "ก.พ."],
    ["2026-12-31", "31", "ธ.ค."],
  ]) {
    const out = dateOnlyTH(d);
    ok(out.includes(day) && out.includes(mon), `${d} แสดงเป็น ${day} ${mon}`, out);
  }
  ok(dateOnlyTH("2026-08-06").includes("2569"), "ปีต้องเป็น พ.ศ.", dateOnlyTH("2026-08-06"));

  // 31 ม.ค. 23:30 เวลาไทย = 16:30Z — ห้ามหลุดไปเดือนถัดไป
  ok(dateTH("2026-01-31T16:30:00Z").includes("31") && dateTH("2026-01-31T16:30:00Z").includes("ม.ค."),
    "31 ม.ค. 23:30 เวลาไทย ยังอยู่เดือน ม.ค.", dateTH("2026-01-31T16:30:00Z"));
  // 1 ก.พ. 00:30 เวลาไทย = 17:30Z ของวันที่ 31 — ต้องข้ามเดือนแล้ว
  ok(dateTH("2026-01-31T17:30:00Z").includes("ก.พ."),
    "1 ก.พ. 00:30 เวลาไทย ข้ามเดือนแล้ว", dateTH("2026-01-31T17:30:00Z"));

  // เครื่องหมาย "รู้แค่วันที่" = เที่ยงตรงเวลาไทย (05:00Z) ต้องไม่โชว์เวลา
  ok(!/\d{2}:\d{2}/.test(dateTH("2026-08-06T05:00:00Z")),
    "วันที่ล้วนต้องไม่โชว์เวลาปลอม", dateTH("2026-08-06T05:00:00Z"));
  ok(/\d{2}:\d{2}/.test(dateTH("2026-08-06T06:15:00Z")),
