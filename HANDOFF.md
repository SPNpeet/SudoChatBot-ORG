...
- `print/[id]`: CSS scale ทั้งใบให้พอดีจอมือถือ (ไม่ต้องเลื่อนซ้ายขวา, พิมพ์จริง scale=1), `table-fixed`+`colgroup`+`tabular-nums` เลขตรงหลักทุกบรรทัด, `@page margin:0` กันล้นหน้า 2 (padding 15mm อยู่ในกระดาษแล้ว), `thead` ซ้ำเมื่อตารางข้ามหน้า, เพิ่มแถว "ยอดชำระจริง" หลังหัก ณ ที่จ่าย
